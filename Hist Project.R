...
stargazer(
  ...
  covariate.labels = c(
    "Daily Stock Returns (SPX)", 
    "Daily Real Estate Returns (VNQ)",
    "30-year Treasury Bond Rate (T30y)",
    "10-year Treasury Note Rate (T10y)",
    "6-month Treasury Bill Rate (T6m)"
  ),
  align = TRUE,
  no.space = TRUE,
  float = TRUE
...
colnames(dfRpre) <- colnames(dfpre)
for (i in 1:ncol(dfpre)){
  dfRpre[1,i]=cor(lagpad(dfpre$VNQ),dfpre[i],use = "complete.obs")
  dfRpre[2,i]=cor(lagpad(dfdur$VNQ),dfdur[i],use = "complete.obs")
  d=var(dfdur[i])/var(dfpre[i])-1
  dfRpre[3,i]=dfRpre[2,i]/sqrt(1+d*(1-dfRpre[2,i]^2))
  z1 <- 0.5 * log((1 + dfRpre[1,i]) / (1 - dfRpre[1,i]))
  z2 <- 0.5 * log((1 + dfRpre[3,i]) / (1 - dfRpre[3,i]))
...
colnames(dfRpost) <- colnames(dfpre)
for (i in 1:ncol(dfpre)){
  dfRpost[1,i]=cor(lagpad(dfpost$VNQ),dfpost[i],use = "complete.obs")
  dfRpost[2,i]=cor(lagpad(dfdur$VNQ),dfdur[i],use = "complete.obs")
  d=var(dfdur[i])/var(dfpre[i])-1
  dfRpost[3,i]=dfRpost[2,i]/sqrt(1+d*(1-dfRpost[2,i]^2))
  z1 <- 0.5 * log((1 + dfRpost[1,i]) / (1 - dfRpost[1,i]))
  z2 <- 0.5 * log((1 + dfRpost[3,i]) / (1 - dfRpost[3,i]))
...
colnames(dfSpre) <- colnames(dfpre)
for (i in 1:ncol(dfpre)){
  dfSpre[1,i]=cor(lagpad(dfpre$SPX),dfpre[i],use = "complete.obs")
  dfSpre[2,i]=cor(lagpad(dfdur$SPX),dfdur[i],use = "complete.obs")
  d=var(dfdur[i])/var(dfpre[i])-1
  dfSpre[3,i]=dfSpre[2,i]/sqrt(1+d*(1-dfSpre[2,i]^2))
  z1 <- 0.5 * log((1 + dfSpre[1,i]) / (1 - dfSpre[1,i]))
  z2 <- 0.5 * log((1 + dfSpre[3,i]) / (1 - dfSpre[3,i]))
...
colnames(dfSpost) <- colnames(dfpre)
for (i in 1:ncol(dfpre)){
  dfSpost[1,i]=cor(lagpad(dfpost$SPX),dfpost[i],use = "complete.obs")
  dfSpost[2,i]=cor(lagpad(dfdur$SPX),dfdur[i],use = "complete.obs")
  d=var(dfdur[i])/var(dfpre[i])-1
  dfSpost[3,i]=dfSpost[2,i]/sqrt(1+d*(1-dfSpost[2,i]^2))
  z1 <- 0.5 * log((1 + dfSpost[1,i]) / (1 - dfSpost[1,i]))
  z2 <- 0.5 * log((1 + dfSpost[3,i]) / (1 - dfSpost[3,i]))
...
write.csv(dfRpre,"dfRpre_lag.csv")
write.csv(dfRpost,"dfRpost_lag.csv")
write.csv(dfSpre,"dfSpre_lag.csv")
write.csv(dfSpost,"dfSpost_lag.csv")
